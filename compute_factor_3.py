...
#from statsmodels.base._constraints import fit_constrained       constrained OLS with equal option only
...
class Data:
    ...
    cal_target_indicators_M.extend([
...
#            'ATR1', 'ATR2'
    ...
    newfinance_indicators = [
            'qfa_oper_rev', 'qfa_oper_cost', 'qfa_net_profit_is',
            'qfa_non_oper_rev', 'qfa_cash_pay_empl', 
            'fix_assets_tot'
            ]
    newfinance_target_indicators = [
            'RROC', 'LPNP', 'OCFA'
            ]
...
#        return series
    ...
    def get_applied_rpt_date_d(self, type_='lyr', real_rpt_date=False):
        if type_ == 'lyr':
            dates = [date for date in self.stm_issuingdate.columns if date.month == 12]
        elif type_ == 'mrq':
            dates = self.stm_issuingdate.columns
        
        res = []
        for date in dates:
            cur_rpt_dat = self.stm_issuingdate[[date]].pivot(columns=date, values=date)
            if not real_rpt_date:
                cur_rpt_dat = cur_rpt_dat.where(pd.isnull(cur_rpt_dat), date)
            res.append(cur_rpt_dat)
        
        re = pd.concat(res, axis=1).dropna(how='all', axis=1).T
        re.index = pd.to_datetime(re.index)
        
        rows = []
        for date in re.index[re.index.duplicated()]:
            row_to_save = re.loc[date].fillna(method='ffill').iloc[-1]
            rows.append(row_to_save)
        
        droped_dup = pd.concat(rows, axis=1).T.drop_duplicates()
        re = re.loc[re.index.difference(droped_dup.index)]
        re = pd.concat([droped_dup, re], axis=0)
        re = re.sort_index()
        assert all(~re.index.duplicated())
        re = re.loc[re.index > pd.to_datetime('1900')]
        alldays = pd.date_range(start=re.index[0], end=pd.datetime.today())
        re = re.reindex(alldays).fillna(method='ffill').loc[self.tradedays].fillna(method='ffill')
        re = re.dropna(how='all', axis=0).T
        
        def fix(series):
            code = series.name
            cmeta = self.meta.loc[code]
            ipo_date = cmeta['ipo_date']
            delist_date = cmeta['delist_date']
            series[series.index < ipo_date] = pd.NaT
            if delist_date:
                series[series.index > delist_date] = pd.NaT
            return series
        
        res_si = []
        for code in re.index:
            res_si.append(fix(re.loc[code]))
        res_si = pd.concat(res_si, axis=1).T
        return res_si
    ...
    def __getattr__(self, name):
        if name not in self.__dict__:
            if name in ('applied_rpt_date_d', 'applied_lyr_date_d'):
                type_ = 'lyr' if name.split("_")[1] == 'lyr' else 'mrq'
                real_rpt_date = name[-1].isnumeric()
                self.__dict__[name] = self.get_applied_rpt_date_d(type_=type_, 
                                             real_rpt_date=real_rpt_date)
            else:
...
class FactorProcess:
    ...
    def _get_trade_days(self, startday, endday, freq=None):
        ...
        except WindQueryFailError as e:
        ...
        if freq == 'd':
            ...
            new_tdays_curfreq = [c_to_t_dict.get(cday, cday) for cday in new_cdays_curfreq]
...
#        tdate = pd.to_datetime('2018-12-31')
#        self = z
        stocklist = self._get_stock_list(tdate)
        df0 = self.meta.loc[stocklist]
        del df0['delist_date']
    ...
    def get_factor_data(self, tdate, stocklist=None):
        ...
        dat_value = self._get_value_data(stocklist, caldate)
        ...
        dat_growth = self._get_growth_data(stocklist, caldate)
        dat_finance = self._get_finance_data(stocklist, caldate)
        dat_leverage = self._get_leverage_data(stocklist, caldate)
        dat_cal = self._get_cal_data(stocklist, tdate)
        dat_tech = self._get_tech_data(stocklist, tdate)
#        dat_newfinance = self._get_newfinance_data(stocklist, tdate)
        res = reduce(self.concat_df, [dat_value, dat_growth, dat_finance, dat_leverage, 
                                      dat_cal, dat_tech])
#        , dat_newfinance])
        if self.updatefreq == 'M':
            dat_barra_quote = self._get_barra_quote_data(stocklist, tdate)
            dat_barra_finance = self._get_barra_finance_data(stocklist, tdate)
            res = reduce(self.concat_df, [res, dat_barra_quote, dat_barra_finance])
        return res
    ...
    def _get_leverage_data(self, stocks, caldate):
        ...
        """        
        date = pd.to_datetime(caldate)
        datdf = pd.DataFrame(index=stocks)
        
        datdf["financial_leverage"] = self.assetstoequity_m.loc[stocks, date]
        datdf["debtequityratio"] = self.longdebttoequity_m.loc[stocks, date]
        datdf["cashratio"] = self.cashtocurrentdebt_m.loc[stocks, date]
        datdf["currentratio"] = self.current_m.loc[stocks, date]
        
        datdf = datdf[self.leverage_target_indicators]
        return datdf
    ...
    def _get_cal_data(self, stocks, tdate):
        ...
        if self.updatefreq == 'w':
            datdf['ln_capital'] = np.log(self.mkt_cap_float_d.loc[stocks, tdate])
            caldate = self.get_last_month_end(tdate)
        else:
            caldate = self.month_map[tdate]
            datdf['ln_capital'] = np.log(self.mkt_cap_float.loc[stocks, caldate])
            
        datdf['holder_avgpctchange'] = self.holder_avgpctchg.loc[stocks, caldate]
        
        mom_vol_data = self._get_mom_vol_data(stocks, tdate, self.dates_d, params=[1,3,6,12])
        turnover_data = self._get_turnover_data(stocks, tdate, self.dates_d, params=[1,3,6,12])
        regress_data = self._get_regress_data(stocks, tdate, self.dates_m, params=["000001.SH", 60])
        
        datdf = reduce(self.concat_df, [datdf, mom_vol_data, turnover_data, regress_data])
        datdf = datdf[self.cal_target_indicators]
        if self.updatefreq == 'M':
            reverse_data = self._get_reverse_data(stocks, tdate, self.dates_d, params=[20,60,180])
            datdf = reduce(self.concat_df, [datdf, reverse_data])
            datdf = datdf[self.cal_target_indicators_M]
        return datdf
        ...
        for tname in self.tech_indicators:
            ...
            if calfunc is None:
                ...
                if tname == "MACD":
                    ...
                    dat['ATR1'], dat['ATR2'] = calfunc(stocks, tdate, self._tech_params[tname])
                else:
...
#            cur_volume = volume.loc[stocks, period_d]
...
#            res[f"vol_std_{offset}m"] = cur_volume_d.apply(self._cal_func, axis=1, args=(np.nanstd,))
        return res
...
#            raise
    ...
    def _get_newfinance_data(self, stocks, tdate):
        """
            Default source data loaded from local file:
            'qfa_oper_rev', 'qfa_oper_cost', 'qfa_net_profit_is',
            'qfa_non_oper_rev', 'qfa_cash_pay_empl', 'fix_assets_tot'
            
            Default target barra_quote indicators:
            "RROC", "LPNP", "OCFA"
        """
#        tdate = pd.to_datetime('2019-05-30'); self = z; stocks = self._get_stock_list(tdate)
#        target_fac, source_fac = 'OCFA', ['qfa_oper_cost', 'fix_assets_tot']

        num_periods = 8
        facs = {
            'RROC': ['qfa_oper_rev', 'qfa_oper_cost'],
            'LPNP': ['qfa_net_profit_is', 'qfa_non_oper_rev', 'qfa_cash_pay_empl'],
            'OCFA': ['qfa_oper_cost', 'fix_assets_tot'],
                }
        mrq_rpt_dates = self.applied_rpt_date_d.loc[stocks, tdate].dropna()
        recent_periods = self._get_recent_finance_panel(mrq_rpt_dates, num_periods)
        res = []
        for target_fac, source_fac in facs.items():
            res.append(self._finance_ts_regress(target_fac, source_fac, recent_periods))
        res = pd.DataFrame(res).T
        return res
    
    def _finance_ts_regress(self, target_fac, source_fac, recent_periods):
        dependant, independants = source_fac[0:1], source_fac[1:]        
        periods = pd.Series(pd.unique(recent_periods.values.flatten())).dropna().sort_values().tolist()
        dependant_dat = self.neu_stan(getattr(self, dependant[0],), dependant, periods)[0]
        independant_dat = self.neu_stan([getattr(self, independant) for independant in independants],
                                         independants, periods)
        res = []
        for code in recent_periods.index:
            regress_dates = recent_periods.loc[code]
            try:
                cur_y = dependant_dat.loc[code, regress_dates] 
                cur_X = pd.concat([indep_dat.loc[code, regress_dates] for indep_dat in independant_dat], axis=1)
            except KeyError:
                res.append(None)
                continue
            try:
                
                coefs = self.regress(cur_X.astype(float), cur_y.astype(float), intercept=False)
            except MissingDataError:
                res.append(None)
                continue
            res.append((cur_y - cur_X @ coefs).iloc[-1])
        res = pd.Series(res, recent_periods.index)
        res.name = target_fac
        return res
        
    def _get_recent_finance_panel(self, mrq_rpt_dates, num_periods):
        stm_idate = self.stm_issuingdate
        res = {}
        for stock in mrq_rpt_dates.index:
            cur_stm_idate = mrq_rpt_dates.loc[stock]
#            if cur_stm_idate.year < 2019:
#                print(stock)
            cur_stk_stm_idates = stm_idate.loc[stock].index.tolist()
            idx = cur_stk_stm_idates.index(cur_stm_idate)
            if idx < num_periods - 1:
                res[stock] = [None] * num_periods
                continue
            cur_stk_recent_periods = cur_stk_stm_idates[idx-7:idx+1]
            if any(stm_idate.loc[stock, cur_stk_recent_periods].isnull()):
                res[stock] = [None] * num_periods
                continue
            res[stock] = cur_stk_recent_periods
        return pd.DataFrame(res).T
    
    def neu_stan(self, data, names, periods):
        if not isinstance(data, list):
            data = [data]
        dat_panel = self._to_panel(data, periods, names)
        for date, panel in dat_panel.items():
#            date1 = pd.to_datetime('2016-09-30')
#            panel = dat_panel[date1]
            panel = panel.dropna(how='any', axis=0)
            panel = panel[panel['industry'] != '0']
            panel = self.neutralize(panel)
            panel[names] = self.standardize(panel[names])
            dat_panel[date] = panel
        data = self._to_matrix(dat_panel)
        return data
    
    def _to_panel(self, dat_matrix, periods, names):
        if not isinstance(dat_matrix, list):
            dat_matrix = [dat_matrix]
        dat_matrix.extend([self.mkt_cap_float, self.industry_citic])
        dat_panel = defaultdict(pd.DataFrame)
        for date in periods:
            for matrix in dat_matrix:    
                dat_panel[date] = pd.concat([dat_panel[date], matrix[date]], axis=1)
            dat_panel[date].columns = names + ['MKT_CAP_FLOAT', 'industry']
        return dat_panel
        
    @staticmethod
    def _to_matrix(dat_panel):
        datpanel = pd.Panel(dat_panel)
        datpanel = datpanel.swapaxes(0, 2)
        dat_matrix = [datpanel.loc[factor] for factor in datpanel.items 
                      if factor not in ('MKT_CAP_FLOAT', 'industry')]
        return dat_matrix
    ...
    def _get_barra_quote_data(self, stocks, tdate):
        ...
        dat_size_barra = self._get_size_barra(stocks, caldate, self.dates_d, params=[True,True,True])
        dat_regress_barra = self._get_regress_barra(stocks, tdate, self.dates_d, params=[4,504,252,True,'000300.SH'])
        dat_dastd_barra = self._get_dastd_barra(stocks, tdate, self.dates_d, params=[252,42])
        dat_cmra_barra = self._get_cmra_barra(stocks, tdate, self.dates_d, params=[12, 21])
        dat_liquidity_barra = self._get_liquidity_barra(stocks, tdate, params=[21,1,3,12])
        dat_rstr_barra = self._get_rstr_barra(stocks, tdate, self.dates_d, params=[252,126,11,'000300.SH'])
        
        datdf = reduce(self.concat_df, [datdf, dat_size_barra, dat_regress_barra, dat_dastd_barra, 
                                      dat_cmra_barra, dat_liquidity_barra, dat_rstr_barra])
        datdf = datdf[self.barra_quote_target_indicators]
        return datdf
    
    def _get_vol_FF_data(self, stocks, tdate):
        """
            Default value indicators getted from windpy:
            "mkt_cap_float", "pb_lf", "pct_chg"
            
            Default target value indicators:
            "std_FF3factor_1m", "std_FF3factor_3m", "std_FF3factor_6m", "std_FF3factor_12m",
            "IVR"
        """
        tdate = pd.to_datetime(tdate)

        dat_std_FF3factor = self._get_std_FF3factor(stocks, tdate, self.dates_d, params=(1,3,6,12))
        dat_ivr = self._get_ivr(stocks, tdate, self.dates_d, params=(20,))
        
        datdf = pd.concat([dat_std_FF3factor, dat_ivr], axis=1)
        datdf = datdf[self.vol_FF_target_indicators]
        return datdf
    
    def _get_std_FF3factor(self, stocks, qdate, dates_d, params=(1,3,6,12)):
        pct_chg = self.pct_chg
        mkt_cap = self.mkt_cap_float_d
        pb = self.pb_lf_d
                
        total = max(params)
        total_period_d = self._get_period_d(qdate, offset=-total, freq="M", datelist=dates_d)
        total_ff3factor = self._get_ff3factor(pct_chg, mkt_cap, pb, total_period_d)
        
        res = pd.DataFrame(index=stocks)
        for offset in params:
            period_d = self._get_period_d(qdate, offset=-offset, freq="M", datelist=dates)
            X = total_ff3factor.loc[period_d]
#            y = pct_chg.loc[:, period_d].T
            
            Xys = pd.concat([X, y], axis=1)
            Xys = Xys.dropna(how='any', axis=0)
            X, y = Xys.iloc[:, :3], Xys.iloc[:, 3:]
            
#            intercept, coef = self.regress(X, y, intercept=True)
            resid = y - (coef * X + intercept)
#            res[f"std_FF3factor_{offset}m"] = np.std(resid, axis=0)
        return res
    
    def _get_ff3factor(self, pct_chg, mkt_cap, pb, period):
        benchmark = '881008.WI'  #万得全A
        mkt = pct_chg.loc[benchmark, period].T
        mkt.name = 'MKT'
        
        res = np.empty((len(period), 2))
        for i, date in enumerate(period):
#            i = 100; date = period[i]
            df = pd.concat([pct_chg[[date]], pb[[date]], mkt_cap[[date]]], axis=1)
            df.columns = ['pct_chg','pb_lf','mkt_cap_float']
            res[i] = self.cal_smb_hml(df)
        
        res = pd.DataFrame(res, columns=['SMB', 'HML'], index=period)
        res = pd.concat([mkt, res], axis=1)
        return res
        
    @staticmethod
    def cal_smb_hml(df):
        # 划分大小市值公司
        df['SB'] = df['mkt_cap_float'].map(lambda x: 'B' if x >= df['mkt_cap_float'].median() else 'S')
        ...
        # 计算各组收益率
        R_SL = (df_SL['pct_chg'] * df_SL['mkt_cap_float']).sum() / df_SL['mkt_cap_float'].sum() 
        R_SM = (df_SM['pct_chg'] * df_SM['mkt_cap_float']).sum() / df_SM['mkt_cap_float'].sum()
        R_SH = (df_SH['pct_chg'] * df_SH['mkt_cap_float']).sum() / df_SH['mkt_cap_float'].sum()
        R_BL = (df_BL['pct_chg'] * df_BL['mkt_cap_float']).sum() / df_BL['mkt_cap_float'].sum()
        R_BM = (df_BM['pct_chg'] * df_BM['mkt_cap_float']).sum() / df_BM['mkt_cap_float'].sum()
        R_BH = (df_BH['pct_chg'] * df_BH['mkt_cap_float']).sum() / df_BH['mkt_cap_float'].sum()
    ...
    def _get_barra_finance_data(self, stocks, tdate):
        ...
        dat_leverage_barra = self._get_leverage_barra(stocks, tdate, self.dates_d)
        dat_value_barra = self._get_value_barra(stocks, caldate)
        dat_growth_barra = self._get_growth_barra(stocks, caldate, params=(5,'y'))

        dat = reduce(self.concat_df, [dat, dat_leverage_barra, dat_value_barra,
                                      dat_growth_barra])
...
#        stocks = self._FactorProcess__get_stock_list(tdate); ori_data = self.current.loc[stocks,:]
    ...
    @staticmethod
    def neutralize(data):
        cols_to_neu = data.columns.difference({'industry', 'MKT_CAP_FLOAT'})
        y = data[cols_to_neu]
        
        lncap = np.log(data[['MKT_CAP_FLOAT']])
        ind_dummy_matrix = pd.get_dummies(data['industry'])
        X = pd.concat([lncap, ind_dummy_matrix], axis=1)
        
        Xys = pd.concat([X, y], axis=1)
        Xys = Xys.dropna(subset=cols_to_neu)
        X, y = Xys.iloc[:, :-len(cols_to_neu)], Xys.iloc[:, -len(cols_to_neu):]
        ...
        if len(cols_to_neu) > 1:
            assert len(data.index.difference(residue.index)) == 0
    
        data.loc[residue.index, y.columns] = residue
        return data
...
#    
#    def cal_ATR(self, stocks, date, params=(20, 60)):
#        n1, n2 = params
#        offset = max(n1, n2) + 1
#        stocks = self._get_stock_list(date)
#        close = self._get_daily_data('hfq_close', stocks, date, offset)
#        high = self._get_daily_data('hfq_high', stocks, date, offset)
#        low = self._get_daily_data('hfq_low', stocks, date, offset)
#        
#        last_close = close.shift(1)
#        tmp1, tmp2, tmp3 = high - low, np.abs(high - last_close), np.abs(low - last_close)
#        tr = pd.Panel({'1': tmp1, '2': tmp2, '3': tmp3})
#        tr = tr.max(axis=0)
#        atr1 = tr.rolling(window=n1).mean()
#        atr2 = tr.rolling(window=n2).mean()
#        return atr1.iloc[-1, :].T.values, atr2.iloc[-1, :].T.values
...
if __name__ == "__main__":
    ...
    w.start()
#    dates = [pd.to_datetime('2018-09-28')]
#    dates = z.month_map['2019-07':].index
    for date in dates:
        try:
            sname = str(date)[:10]
