...
from utility.constant import NH_index_dict, code_name_map_citic, code_name_map_sw, index_code_name_map
from WindPy import *

token = 'a181752ac7109af4ce60972a2be1ef7a190f272fac6fd41b1a6836cf'
...
def update_stock_future_dat():
    ...
    his_df = pro.fut_basic(exchange='CFFEX', fields='ts_code,symbol,name,list_date,delist_date')
    ...
    his_df = his_df.drop(['symbol', 'name'], axis=1)
    ...
    for i, se in his_df.iterrows():
        ...
        name = se['ts_code'].split('.')[0]
        data.save(res_tmp,  name, save_path=p)
        time.sleep(0.6)
